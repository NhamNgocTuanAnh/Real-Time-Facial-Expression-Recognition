from keras.preprocessing.image import img_to_array
import imutils
import pytz

UTC = pytz.timezone('UTC')  # utc
from datetime import datetime as dt
import time
from time import sleep
import cv2, os
import csv
from csv import DictWriter
from keras.models import load_model
import numpy as np
import pandas as pd
from keras.models import model_from_json
from tensorflow.compat.v1 import InteractiveSession
from tensorflow.compat.v1 import ConfigProto

config = ConfigProto()
config.gpu_options.allow_growth = True
session = InteractiveSession(config=config)

# parameters for loading data and images
detection_model_path = 'haarcascade_frontalface_default.xml'
emotion_model_path = 'models/_mini_XCEPTION.106-0.65.hdf5'
# load model facial_expression
model_facial_expression = model_from_json(open("facial_expression_model_structure.json", "r").read())

# load weights facial_expression
model_facial_expression.load_weights('facial_expression_model_weights.h5')

# hyper-parameters for bounding boxes shape
# loading models
face_detection = cv2.CascadeClassifier(detection_model_path)
emotion_classifier = load_model(emotion_model_path, compile=False)
EMOTIONS = ["angry", "disgust", "scared", "happy", "sad", "surprised", "neutral"]
field_names = ["Timestamp", "angry", "disgust", "scared", "happy", "sad", "surprised", "neutral"]


# find a large enough sample size
def SlovinFormula(number_face, confidence_level):
    return int(number_face / (number_face * pow(confidence_level, 2) + 1))


# starting video streaming
path_video = 'democlassroom.mp4'
path_video2 = 'Real_Time_Expression.mp4'

cv2.namedWindow('your_face')
camera = cv2.VideoCapture(path_video)

width = int(camera.get(cv2.CAP_PROP_FRAME_WIDTH) + 0.5)
height = int(camera.get(cv2.CAP_PROP_FRAME_HEIGHT) + 0.5)
size = (width, height)
fourcc = cv2.VideoWriter_fourcc(*'XVID')
out = cv2.VideoWriter('your_video.avi', fourcc, 20.0, size)

while True:
    frame = camera.read()[1]
    if np.shape(frame) == ():
        print("Empty frame!")
    else:
        # reading the frame
        # frame = imutils.resize(frame,width=800)
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        faces = face_detection.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30),
                                                flags=cv2.CASCADE_SCALE_IMAGE)

        canvas = np.zeros((550, 300, 3), dtype="uint8")
        canvas_fake = np.ones((330, 660, 3))*255
        frameClone = frame.copy()
        total_preds = np.array([])

        if len(faces) > 0:
            # faces = sorted(faces, reverse=True,
            # key=lambda x: (x[2] - x[0]) * (x[3] - x[1]))[0]
            # total_preds =
            count = 0
            break_point = 0
            break_hack = 0
            if len(faces) > 10:
                break_point = SlovinFormula(len(faces), 0.2)
                print("So mau lay thuc la: " + str(break_point))
            for (fX, fY, fW, fH) in faces:
                # Extract the ROI of the face from the grayscale image, resize it to a fixed 48x48 pixels, and then prepare
                # the ROI for classification via the CNN
                roi = gray[fY:fY + fH, fX:fX + fW]
                roi = cv2.resize(roi, (48, 48))
                roi = roi.astype("float") / 255.0
                roi = img_to_array(roi)
                roi = np.expand_dims(roi, axis=0)

                preds = model_facial_expression.predict(roi)[0]
                emotion_probability = np.max(preds)
                label = EMOTIONS[preds.argmax()]
                if count == 0:
                    total_preds = preds
                else:
                    total_preds += preds
                count += 1
                break_point -= 1
                break_hack = count
                for (i, (emotion, prob)) in enumerate(zip(EMOTIONS, preds)):
                    # construct the label text
                    text = "{}: {:.2f}%".format(emotion, prob * 100)
                    cv2.putText(frameClone, label, (fX, fY - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
                    cv2.rectangle(frameClone, (fX, fY), (fX + fW, fY + fH),
                                  (0, 0, 255), 2)
                if break_point == 0 or break_hack == 3:
                    break
            total_preds = total_preds / len(faces)
            cv2.putText(frameClone, "Class size:" + str(len(faces)), (40, 177), cv2.FONT_HERSHEY_SIMPLEX, 3,
                        (100, 255, 0), 3, cv2.LINE_AA)

            exists = os.path.isfile("data.csv")
            if exists:
                df = pd.read_csv("data.csv")
            else:
                print('Details Missing')
            ts = time.time()
            now = dt.now(pytz.timezone('Asia/Ho_Chi_Minh'))
            time_now = now.strftime("%d-%m-%Y %H:%M:%S")
            date = dt.fromtimestamp(ts).strftime('%d-%m-%Y')
            dict_temp = {}

            for (i, (emotion, prob)) in enumerate(zip(EMOTIONS, total_preds)):
                dict_temp[i] = prob * 100
                # construct the label text
                text = "{}: {:.2f}%".format(emotion, prob * 100)
                w = int(prob * 300)
                cv2.rectangle(canvas, (7, (i * 35) + 5),
                              (w, (i * 35) + 35), (0, 0, 255), -1)
                cv2.putText(canvas, text, (10, (i * 35) + 23),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.45,
                            (255, 255, 255), 2)
            dict = {'Timestamp': str(time_now), 'angry': str(dict_temp[0]), 'disgust': str(dict_temp[1]),
                    'scared': str(dict_temp[2]), 'happy': str(dict_temp[3]),
                    'sad': str(dict_temp[4]), 'surprised': str(dict_temp[5]), 'neutral': str(dict_temp[6])}
            with open('data.csv', 'a') as f_object:
                dictwriter_object = DictWriter(f_object, fieldnames=field_names)
                dictwriter_object.writerow(dict)
                f_object.close()

        cv2.imshow('your_face', frameClone)
        cv2.imshow("Probabilities", canvas)
        cv2.imshow("Probabilities_fake", canvas_fake)
        out.write(frameClone)
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

camera.release()
out.release()
cv2.destroyAllWindows()
